...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  {
    /* NET_PRINT("cannot transmit wifi not rdy\n"); */

    /* Stop lint warning about packet not being freed - it is not being referenced */ /*@-mustfree@*/
    err = (err_t) ERR_INPROGRESS; /* Note that signalling ERR_CLSD or ERR_CONN causes loss of connectivity on a roam */
    /*@+mustfree@*/
  }
...
int32_t  mx_wifi_if_init(net_if_handle_t *pnetif)
{
  ...
  if (ptmp != NULL)
  {
    (void)memcpy((void *)&p, (void *)&ptmp, sizeof(p));
    ...
    {
      (void)memcpy((void *) & (p->extension.wifi), (void *)&ptmp, sizeof(p->extension.wifi));
      /* dhcp mode */
...
static  int32_t hw_start(net_if_handle_t *pnetif)
{
  ...
  if (mxwifi_probe(&pnetif->pdrv->context) == NET_OK)
  {
    ...
    if (pMxWifiObj->Runtime.interfaces == 0)
    {
...
static int32_t mx_wifi_if_start_softap(net_if_handle_t *pnetif)
{
  ...
  (void) sprintf(ap_cfg.ip.gateway, "%ld.%ld.%ld.%ld", ADDR(pnetif->static_gateway.addr));
  (void) sprintf(ap_cfg.ip.netmask, "%ld.%ld.%ld.%ld", ADDR(pnetif->static_netmask.addr));
                                       ...
                                       uint8_t scan_bss_array_size)
{
  ...
  else
  {
    ...
    void *ptmp = NET_MALLOC(sizeof(mwifi_ap_info_t) * number);
    ...
    else
    {
      ...
      (void) memcpy((void *) &ap_list_head, (void *) &ptmp, sizeof(ap_list_head));
      ap_info = ap_list_head;
      (void) memset(ap_list_head, 0, sizeof(mwifi_ap_info_t) * number);
...
static int32_t mx_wifi_getsockname(int32_t sock, net_sockaddr_t *name, uint32_t *namelen)
{
  ...
  struct sockaddr *mx_addr;
  (void)memcpy(&mx_addr, (struct sockaddr *) &name, sizeof(mx_addr));
