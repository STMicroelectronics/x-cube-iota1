...
/* Private functions -------------------------------------------------------- */
void test_find_msg_by_index(void)
{
  iota_client_conf_t ctx = {.host = TEST_NODE_HOST, .port = TEST_NODE_PORT, .use_tls = TEST_IS_HTTPS};

  res_find_msg_t* res = res_find_msg_new();
  TEST_ASSERT(find_message_by_index(&ctx, "st_disco", res) == 0);
  TEST_ASSERT(res->is_error == false);
  res_find_msg_free(res);
  res = NULL;

//  res = res_find_msg_new();
//  char index[]="iota.c\xF0\x9F\x80\x84";
//  printf("strlen(index)=%d", strlen(index));
//  TEST_ASSERT(find_message_by_index(&ctx, "iota.c\xF0\x9F\x80\x84", res) == 0);
//  TEST_ASSERT(res->is_error == false);
//  res_find_msg_free(res);
}

void test_deser_find_msg(void)
{
