...
/* Private functions -------------------------------------------------------- */
void test_send_indexation(void)
{
  iota_client_conf_t ctx = {.host = TEST_NODE_HOST, .port = TEST_NODE_PORT, .use_tls = TEST_IS_HTTPS};

  res_send_message_t res;
  memset(&res, 0, sizeof(res_send_message_t));
  TEST_ASSERT(send_indexation_msg(&ctx, "st_disco", "Hello IOTA", &res) == 0);
  TEST_ASSERT_FALSE(res.is_error);
  printf("message ID: %s\n", res.u.msg_id);
}

void test_send_core_message_indexation(void)
{
  iota_client_conf_t ctx = {.host = TEST_NODE_HOST, .port = TEST_NODE_PORT, .use_tls = TEST_IS_HTTPS};
...
void test_serialize_indexation(void)
{
...
void test_deser_send_msg_response(void)
{
...
void test_send_core_message_tx(void)
{
  ...
  memset(&genesis_seed_keypair, 0, sizeof(iota_keypair_t));
  TEST_ASSERT(hex_2_bin("f7868ab6bb55800b77b8b74191ad8285a9bf428ace579d541fda47661803ff44", 64, genesis_seed_keypair.pub,
                      ED_PUBLIC_KEY_BYTES) == 0);
  TEST_ASSERT(hex_2_bin("256a818b2aac458941f7274985a410e57fb750f3a3a67969ece5bd9ae7eef5b2f7868ab6bb55800b77b8b74191ad8285"
  ...
  memset(genesis_addr, 0, sizeof(genesis_addr));
  ...
  for (size_t i = 0; i < out_counts; i++) {
    ...
    memset(&res_out, 0, sizeof(res_output_t));
