...
#else
static ed25519_vector_t* ed25519_vector_new(void)
{
...
void test_address_gen()
{
  ...
  memset(&seed_keypair, 0, sizeof(iota_keypair_t));
...
void test_sha()
{
  uint8_t tmp_bin[CRYPTO_SHA512_HASH_BYTES] = {0};
  uint8_t tmp_hash[CRYPTO_SHA512_HASH_BYTES] = {0};
...
int test_crypto(void)
{
  ...
  printf("test_sha exp time[ms]: %lu\n", end - start);
#endif /* EXPENDED_TEST_TIME */
