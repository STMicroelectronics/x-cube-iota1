...
/* Constants -----------------------------------------------------------------*/
#define MSG_DATA "SENSOR DATA (IOTA L2Sec app payload"
#define STR_BUFFER_SIZE (16384 + 1)

/* type 2 payload max sizes */
#define MAX_DATA_HEX_LENGTH (32768 + 1) // TODO: find the macro, if any 
#define MAX_INDEX_HEX_LENGTH (64 + 1)  // TODO: find the macro, if any 

/* FIXME: TO BE REMOVED */
#define MAX_DATA_LENGTH (16384 + 1) // TODO: find the macro, if any 
#define MAX_INDEX_LENGTH (32 + 1)  // TODO: find the macro, if any
                            ...
                            byte_t* index)
{
  ...
  uint16_t data_length_nob = NET_HTONS (data_length); /* convert 2 net order byte */
  byte_buf_append(buffer, (byte_t const*)&data_length_nob, sizeof(data_length_nob));
...
#ifdef IOTA_L2_MSG_DEBUG
  printf("\nAdded public key to buffer (length: %lu)\n", sizeof(keypair.pub));
  byte_buf_print(buffer);
#endif /* IOTA_L2_MSG_DEBUG */
...
#ifdef IOTA_L2_MSG_DEBUG
  printf("\nAdded signature to buffer (length: %lu)\n", sizeof(signature));
                              ...
                              byte_t *recv_next_index)
{
  ...
  uint16_t recv_data_length_nob; 
  memcpy((byte_t *) &recv_data_length_nob, buffer->data, 
         sizeof(recv_data_length_nob));
...
static int send_l1_message(byte_buf_t* app_data, byte_t* index, char* msg_id)
{
  ...
  do {
    ...
    rtx++;
    if ((err == HTTP_ERR_SEND) || (err == HTTP_ERR_OPEN)) {
      printf("[%s:%d]: Message rtx #%d\n", __func__, __LINE__, rtx);
    }

  } while (((err == HTTP_ERR_SEND) || (err == HTTP_ERR_OPEN)) && (rtx <= MAX_L1_RETRY));
  ...
  if (msg->is_error) {
    ...
    switch (msg->u.msg->type) {
      case MSG_PAYLOAD_TRANSACTION:
        err = -2;
        printf("[%s:%d]: ERROR message type: found transaction instead of indexation\n", __func__, __LINE__);
        break;
...
#ifdef IOTA_L1_MSG_DEBUG
        printf("Found message - type: indexation (%d)\n", MSG_PAYLOAD_INDEXATION);
#endif /* IOTA_L1_MSG_DEBUG */
        err = 0; //FIXME: weird issue - without this statement and with IOTA_L1_MSG_DEBUG not defined, compiling the code fails
	...
	data[buf_data->len] = '\0';
        ...
        //printf("\nReceived hex encoded L2Sec wrapped data - copied to char buffer):\n%s\n",data);
      ...
      case MSG_PAYLOAD_MILESTONE:
        err = -3;
        printf("[%s:%d]: ERROR message type: found milestone instead of indexation\n", __func__, __LINE__);
        break;
...
static int get_l1_message_by_id (char *msg_id, byte_buf_t* app_data, byte_t* index)
{
  ...
  char received_data_string[MAX_DATA_LENGTH];
  char received_index_string[MAX_INDEX_LENGTH];
...
static int get_app_data (byte_buf_t* data_to_send, int n_iter)
{
  ...
  if (current_time == ((time_t)-1)) {
    ...
    c_time_string[strlen(c_time_string) - 1] = '\0'; // remove the trailig newline
  ...
  if (notime) { // could not get or convert the time
    // FIXME: add error checking (negative number)
    snprintf (str_buffer, STR_BUFFER_SIZE, "%s #%d) -- %s\n",
              MSG_DATA,
              n_iter + 1,
              "NO TIME");
  }
  ...
  if(SensorDataToJSON(str_buffer, STR_BUFFER_SIZE) != 0) {
                                            ...
                                            int n_iter)
{
  ...
  printf ("Application data message to L2Sec protect and send: \n'%s'\n\n", data->data);
...
static void example_send_l2sec_protected_stream(int n_messages, byte_t *first_index)
{
  ...
  if(n_messages < 0) {
    ...
    printf("\n\nA negative value has been entered. Message nember set to default %d \n\n", n_messages);
  }
