...
uint8_t links_psk_wrapped[L2SEC_PSK_WRAPPED_SIZE] = {0};   // Wrapped PSK
/* *********************** STSAFE-L2SEC Integration ************************ */
...
static uint8_t stsafe_PSK_init(void)
{
  ...
  printf("STSAFE: (demo-only) PSK is reset ");
  StatusCode = stsafe_PSK_deinit();
  printf("-> OK\n");

  if(!stsafe_PSK_isStored()){
    printf("STSAFE: PSK in STSAFE-NVM is empty\n");
    ...
    printf("STSAFE: Writing PSK to STSAFE-NVM ");
    StatusCode = stsafe_PSK_sec_store(tmp_plain_psk);
    printf("-> OK\n");
  }

  // Secure Load = Load(Wrapped PSK) from NVM to RAM
  printf("STSAFE: Loading Wrapped PSK from STSAFE-NVM to memory ");
...
/*************************** Echo ************************/
int stsafe_echo(void)
{
  StSafeA_LVBuffer_t sts_echo_out;
  int ret;

  uint8_t pInEchoData[] = "TEST Echo";
  uint16_t RespDataLen = sizeof(pInEchoData);
  
  uint8_t data_sts_echo_out [sizeof(pInEchoData)];
  sts_echo_out.Length = sizeof(pInEchoData);
  sts_echo_out.Data = data_sts_echo_out;

  if ((ret = StSafeA_Echo(&stsafea_handle, pInEchoData, RespDataLen,
                          &sts_echo_out, STSAFEA_MAC_NONE)) == STSAFEA_OK)
  {                          
    char out_buffer [sizeof(pInEchoData)];
    memset (out_buffer, 0, sizeof(pInEchoData));
    
    snprintf(out_buffer, sts_echo_out.Length, (char *) sts_echo_out.Data);
    //snprintf(out_buffer, sizeof("123456789"), "123456789");
    
    printf("Sent string for echoing:  %s\n\r", pInEchoData);
    printf("STSAFE's returned output: %s\n\r\n\r", out_buffer);
  }
  else
  {
    printf("ERROR: StSafeA_Echo error number %d\n", ret);
  }
